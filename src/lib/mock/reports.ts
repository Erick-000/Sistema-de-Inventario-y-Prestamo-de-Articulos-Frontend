import type { LoanRequest } from "@/lib/mock/requests";
...
export function getLateReturnsSummary(requests: LoanRequest[]) {
  const overdue = requests.filter((r) => r.status === "VENCIDO").length;
  const returned = requests.filter((r) => r.status === "DEVUELTO").length;
  const active = requests.filter((r) => r.status === "ACTIVO").length;
